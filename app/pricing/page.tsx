    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">Manage delivery pricing per vehicle type</p>
        </div>
        <div className="flex gap-3">
          <button onClick={() => setShowEstimate(true)} className="flex items-center gap-2 px-4 py-2 bg-white border border-gray-200 rounded-lg text-sm font-medium hover:bg-gray-50">
            <Calculator className="w-4 h-4" /> Test Estimate
          </button>
          <button onClick={() => setShowCreate(true)} className="flex items-center gap-2 px-4 py-2 bg-brand-600 text-white rounded-lg text-sm font-medium hover:bg-brand-700">
      ...
      <Modal open={showCreate} onClose={() => setShowCreate(false)} title="Create Pricing Rule">
        <div className="space-y-4">
          ...
          <button onClick={handleCreateRule} className="w-full py-2.5 bg-brand-600 text-white rounded-lg text-sm font-medium hover:bg-brand-700">Create Rule</button>
        </div>
      </Modal>
      ...
      <Modal open={showEstimate} onClose={() => { setShowEstimate(false); setEstimateResult(null); }} title="Price Estimate Tester">
        <div className="space-y-4">
          ...
          <button onClick={handleEstimate} className="w-full py-2.5 bg-brand-600 text-white rounded-lg text-sm font-medium hover:bg-brand-700">Calculate</button>
          ...
          {estimateResult && (
            <div className="bg-gray-50 rounded-xl p-4 space-y-2 mt-4">
              ...
              <div className="flex justify-between text-base font-bold border-t pt-2 mt-2">
                <span>Total</span>
                <span className="text-brand-600">{formatCurrency(estimateResult.total_price)}</span>
              </div>
            </div>
          )}
        </div>
